...
public class UserAuthServiceImpl extends ServiceImpl<UserAuthDao, UserAuth> implements UserAuthService {
    ...
    public void sendCode(String username) {
        ...
        message.setFrom("1819855758@qq.com");
    ...
    public void updateAdminPassword(PasswordVO passwordVO) {
        ...
        UserAuth userAuth = userAuthDao.selectOne(queryWrapper.eq("id", UserUtil.getLoginUser()
                .getId()));
        // 如果用户认证信息不存在，则无法继续
        if (userAuth == null) {
        ...
        if (BCrypt.checkpw(passwordVO.getOldPassword(), userAuth.getPassword())) {
            // 旧密码正确，更新新的哈希密码
            String newHashedPassword = BCrypt.hashpw(passwordVO.getNewPassword(), BCrypt.gensalt());
            userAuth.setPassword(newHashedPassword);
